...
Babel compiles JSX to the equivalent with `React.createElement` and
`ReactDOM.render`. If we use interpolation for a prop or the children, babel
doesn't do anything with it and just pass it as a prop to `React.createElement`

If we spread props to an element, passing nothing else, babel knows that it just
has to pass the whole props object to `React.createElement`. But if we spread
some props alongside with other individual props, babel adds a method to the
compiled code to merge objects. It merges the individual and spread props, in
the order they are declared in the JSX (which is important if we want to
overwrite a prop from the spread)
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Fundamentals%20%E2%9A%9B&e=03%3A%20Using%20JSX&em=yoh.legrand%40gmail.com).
